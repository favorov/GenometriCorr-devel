...
\item For the p-values of the permutation distributions, we also want to know whether the query and reference intervals are significantly close together (attration) or significantly far apart (repulsion). If the alternative was stated by the  {\tt alternative} parameter ({\tt 'attraction'} (default) of {\tt repulsion}), it is passed to the {\tt direction} field in the result (namely, {\tt jaccard.measure.test.direction} and {\tt scaled.absolute.min.distance.sum.test.direction}). If the {\tt alternative} is {\tt 'two.sided'} the values of the {\tt jaccard.measure.test.direction} and {\tt scaled.absolute.min.distance.sum.test.direction} in the return result show the direction of the effect.
\end{itemize}
In the last run, the {\tt alternative} was the default {\tt 'attraction'} all tests indicate that the query and reference intervals are close together and/or overlapping. Let's rerun it with {\tt 'two.sided'}.

<<test_chromosome_123_two.sided,cache=TRUE>>=

cpgi_to_genes_two.sided<-GenometriCorrelation(cpgis,refseq,chromosomes.to.proceed=c('chr1','chr2','chr3'),permut.number=permut.number,keep.distributions=TRUE,showProgressBar=FALSE,alternative='two.sided')
@

The result is:

<<report_chromosome_123_two.sided,cache=TRUE>>=
print(cpgi_to_genes_two.sided)
@

We see, that the direction is attraction. Now, it tells us real effect sign rather than just shows the alternative we asked for. The p-value now are like two time more for the two.sided alternative. 
