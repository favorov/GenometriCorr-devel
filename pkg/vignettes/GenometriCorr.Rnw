...
The information about lengths and names of chromosomes is coded in seqinfo provided by TxDb.Hsapiens.UCSC.hg19.knownGene. The refseq object contains it; the cpgis is read from a bed file and thus we set the seqinfo. If the query and reference oblects do not carry seqinfo, the information can be passed via {\tt chromosomes.length} parameter that is organised exactly as {\tt seqlenghts(GenomicRanges)} result. It is, a numeric vector with names, whic are chromosome names.

If the chromosome lengths are not either given explicitely or coded in reference or in query, they are taken to be the maximum of the reference and query coordinates provided, per chromosome, and a warning is generated. You can suppress the warning with {\tt supress.evaluated.length.warning=TRUE}.
